...
for time_point in range(n_timepoints):
    ...
    mean_genes_shap = np.array(np.array_split(genes_shap, N_FOLDS, axis=0)).mean(axis=0).sum(axis=1)
    mean_metab_shap = np.array(np.array_split(metab_shap, N_FOLDS, axis=0)).mean(axis=0).sum(axis=1)
...
for time_point in range(n_timepoints):
    ...
    for patient_id in range(n_individuals):
        ...
        patient_shap = np.concatenate([
            patient_shap_genes_cluster,
            patient_shap_metab_cluster,
            patient_shap_clin,
            patient_shap_baseline
            ], axis=0
        )
