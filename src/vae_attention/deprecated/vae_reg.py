...
y, X, Z = data_generation(n, k, p, noise_scale = 0.5)
...
class SupVAE(nn.Module):
    def __init__(self, input_dim, latent_dim, dropout_sigma=0.1):
        ...
        self.top = nn.Linear(latent_dim, 1)
    ...
    def linear_top(self, z):
        return self.top(z)

    def forward(self, x):
        mu, logvar = self.encode(x)
        z_hat = self.reparameterize(mu, logvar)
        y_hat = self.linear_top(z_hat)
    ...
    def loss(self, m_out, x, y, beta=1.0):
...
latent_dim = k
model = SupVAE(input_dim=p, latent_dim=latent_dim, dropout_sigma=0.2).to(device)
...
imputer = sage.MarginalImputer(predict, data_train[0])  # Background data
