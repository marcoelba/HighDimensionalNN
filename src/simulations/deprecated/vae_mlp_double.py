...
# VAE + regression
class VAE_MLP_Z(nn.Module):
    def __init__(self, input_dim, latent_dim, dropout_sigma=0.1, beta_vae=1.0, alpha=0.9):
        super(VAE_MLP_Z, self).__init__()
    ...
    def y_pred_layer(self, x_hat, z_hat):
        y_hat_from_x = self.top(x_hat)
        y_hat_from_z = self.z_to_y(z_hat)

        return self.alpha * y_hat_from_x + (1 - self.alpha) * y_hat_from_z

    def forward(self, x):
        mu, logvar = self.encode(x)
        z_hat = self.reparameterize(mu, logvar)
        x_hat = self.decode(z_hat)
        y_hat = self.y_pred_layer(x_hat, z_hat)
...
latent_dim = k
model = VAE_MLP_Z(
    input_dim=p, latent_dim=latent_dim,
    dropout_sigma=0.2, beta_vae=0.5, alpha=0.5
).to(device)
