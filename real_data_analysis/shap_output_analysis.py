...
time_all_genes_shap.shape
sorted_genes = np.argsort(np.abs(time_all_genes_shap).sum(axis=0).sum(axis=1))[::-1]

top_genes = 10

for gene in range(top_genes):
    time_genes_shap = np.concatenate([shap_values[tt][0][:, gene] for tt in range(n_timepoints)], axis=1)
    fig = plt.figure()
    shap.summary_plot(
        genes_shap[:, [sorted_genes[gene]], :],
        features=background_data[0][[sorted_genes[gene]]],
        feature_names=[f"time_{tt}" for tt in range(n_timepoints)],
        show=False
    )
    fig.savefig(f"{PATH_PLOTS}/genes_shap_top_{gene}.pdf", format="pdf")
...
pat_features = background_data[1]
